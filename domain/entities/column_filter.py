...
SqlToolAdapter._get_filter is already a dictionary lookup over a `filters`
argument taken in the constructor, so the adapter itself is finished. What is
missing is the thing that builds that dictionary without anyone hand-writing
it - which is what this entity and the classifier are for.

The old codebase kept four hand-maintained lists (semantic, word, operation,
datetime) and rescanned all four on every call. Almost all of that is
recoverable from the database instead:
